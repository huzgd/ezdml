...
type TBESENGarbageCollectorObjectList=class;
     ...
     TBESENGarbageCollector=class(TBESENBaseObject)
      public
       ...
       Count:int64;
       FCollectingAll: Boolean; //added by huz 20191007
       constructor Create(AInstance:TObject); overload; override;
       destructor Destroy; override;
       procedure Clear;
       procedure Reset;
...
begin
 result:=false;
 if not FCollectingAll then //added by huz 20191003
   Exit;
 while true do begin
  case State of
   bgcsINIT:begin
...
begin
  if FCollectingAll then
    Exit;
  FCollectingAll:=True;   //added by huz 20191007
  try
     while Collect do begin
     end;
  finally
    FCollectingAll:=False;
  end;
end;
