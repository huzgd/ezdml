...
uses Classes,TypInfo,Variants,BESENConstants,BESENTypes,BESENObject,BESENObjectFunction,BESENValue,BESENObjectPropertyDescriptor,
     BESENEnvironmentRecord, SysUtils;
...
type TBESENNativeObject=class(TBESENObjectFunction)
      ...
      protected
       FSkipPropMark: Boolean; //added by huz 20191101
       ...
       function ConvertBsPropResObj(obj: TObject): TObject; virtual; //added by huz 20191002
       function ConvertBsPropValToObj(const bsVal: TBESENValue): TObject; virtual; //added by huz 20191002
      public
...
uses {$ifdef BESENEmbarcaderoNextGen}System.SysUtils,{$endif}BESEN,BESENErrors,BESENHashUtils,BESENStringUtils,BESENGarbageCollector,BESENCollectorObject;
   ...
   for i:=0 to PropListLen-1 do begin
    ...
    if assigned(Item^.PropType) then begin
     case Item^.PropType^.Kind of
      ...
      tkInt64{$ifdef fpc},tkQWORD,tkBool{$endif},tkClass:begin
{$ifdef BESENEmbarcaderoNextGen}
...
function TBESENNativeObject.ConvertBsPropResObj(obj: TObject): TObject;  //added by huz 20191002
begin
  Result := obj;
end;

function TBESENNativeObject.ConvertBsPropValToObj(const bsVal: TBESENValue): TObject;  //added by huz 20191002
begin
  Result := TBESEN(Instance).ToObj(bsVal);
end;
...
{$ifdef fpc} //added by huz 20200412
  tkBool:     
   begin
     V.ValueType := bvtBOOLEAN;
     V.Bool := StrToBool(GetEnumProp(self,Item));  
   end;
{$endif}
  tkEnumeration:begin
   if Item^.PropType^.Name = 'Boolean' then
   begin  //added by huz 20191006
     V.ValueType := bvtBOOLEAN;
     V.Bool := StrToBool(GetEnumProp(self,Item));
   end
   else
  ...
  tkClass:begin
   ...
   if assigned(O) then begin
     O := ConvertBsPropResObj(O); //added by huz 20191002
...
{$ifdef fpc} //added by huz 20200412
  tkBool:
   begin                               
     //SetOrdProp(self,Item,TBESEN(Instance).ToBool(V));
    SetEnumProp(self,Item,TBESEN(Instance).ToStr(V));
   end;
{$endif}
  tkEnumeration:begin
   if (V.ValueType=bvtNUMBER) then //added by huz 20191002
     SetOrdProp(Self, Item, Round(V.Num))
   else if StrToIntDef(TBESEN(Instance).ToStr(V), -1)>=0 then
     SetOrdProp(Self, Item, StrToIntDef(TBESEN(Instance).ToStr(V), -1))
   else
  ...
  tkClass:begin     
     O := ConvertBsPropValToObj(V);//added by huz 20191002
   SetObjectProp(self,Item,O); //TBESEN(Instance).ToObj(V)
  end;
  tkVariant:begin
...
var i:integer;
    ...
    Item:PPropInfo;
begin
 if assigned(PropList) and not FSkipPropMark then begin
  for i:=0 to PropListLen-1 do begin
   Item:=PropList^[i];
   if assigned(Item^.PropType) then begin
    case Item^.PropType^.Kind of
